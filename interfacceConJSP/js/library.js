...
function insert() {
	...
	var date = document.insert_product_form.date.value;
	var category = document.insert_product_form.category.value;
    ...
    datev = new Date(datev[2], date[1] - 1, datev[0]);
	var priceRe = /^\d+(?:[.,]\d+)*$/;
	...
	if(image == null || image==""){
		...
		document.getElementById("insert_error").innerHTML = "Il titolo non può essere vuoto!";
		return false;
		...
		document.getElementById("insert_error").innerHTML = "Il campo autori non può essere vuoto!";
		return false;
	}else if(date=="" || date==null || !dateRe.test(date) || (today-datev) < 0){
		...
		document.getElementById("insert_error").innerHTML = "Il campo descrizione non può essere vuoto!";
		return false;
	}
	else
	{
